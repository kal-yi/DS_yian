...
from sklearn.preprocessing import OneHotEncoder, LabelEncoder, PowerTransformer, QuantileTransformer
...
def model_and_clf(preprocessor, n_estimators=250, max_depth=20): #default n_estimators, max_depth - unless overwritten
...
def get_preds(preprocessor, X_train, X_valid, y_train, n_estimators, max_depth): #X_Valid or X_test to return preds

    # Create classifier
    clf1=model_and_clf(preprocessor, n_estimators, max_depth)
...
def RF_params(preprocessor, X, y):
    ...
    results= {}
    for i in range(10,21,5): #n_estimators
        for j in range(2,7,2): #max_depth
            clf2 = model_and_clf(preprocessor, i, j)
            MAE_param = cross_validation(clf2, X, y)
            results["n_estimators" + str(i), "max_depth="+ str(j)] = MAE_param

    results_sorted = {k: v for k, v in sorted(results.items(), key=lambda item: item[1])}

    return results_sorted

def create_X_test_pred(preprocessor, X_train, X_test, y_train, n_estimators, max_depth):

    # Function to create the .csv document with predictions

    # Get predictions for X_test data
    preds = get_preds(preprocessor, X_train, X_test, y_train, n_estimators, max_depth)

    # Create .csv document
    output = pd.DataFrame({'Id': X_test.index,
                           'SalePrice': preds})
    output.to_csv('Try2.csv', index=False)
...
def Housing_Prices():
    ...
    preds = get_preds(preprocessor,X_train, X_valid, y_train, 20, 5) # n_estimators, max_depth
    ...
    pred_file = create_X_test_pred(preprocessor, X_train, X_test, y_train, 250, 20) # n_estimators, max_depth
